...
function AddFreight() {
  ...
  const interactContract = async () => {
    try {
      ...
      let fine_delivery_late = ethers.utils.parseEther(
        values.fine_delivery_late
      );
      let guarantee_value = ethers.utils.parseEther(values.guarantee_value);

      // parse datetimes (UNIX timestamps)
      let date_limit_get_load = Math.floor(
        new Date(dates.date_limit_get_load).getTime() / 1000
      );
      let date_limit_delivery = Math.floor(
        new Date(dates.date_limit_delivery).getTime() / 1000
      );
      ...
      if (ethereum) {
        ...
        let last_freight = await freightFactory.getLastFreightOwner(
          currentAccount
        );
  ...
  return (
    <div>
      <h1>Add freight</h1>
      <small>
        In order to make a new freight, you need to provide the next details:{" "}
      </small>
